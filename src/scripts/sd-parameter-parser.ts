...
export class SdParameterParser {
  /**
   * SDタグ文字列をパースする
   */
  static parseSdTags(str: string): SdTag[] {
  ...
  /**
   * キー・バリューペアのテキストをパースする
   */
  private static parseKeyValuePairs(text: string): [string, string][] {
   ...
   */
  static parseSdParameters(parameter: string): SdParameters {
    ...
    const positiveSdTags = this.parseSdTags(ppSeparated[0]);
    const negativeSdTags = this.parseSdTags(npSeparated[0]);

    const fieldEntries = this.parseKeyValuePairs("Steps:" + npSeparated[1]);
